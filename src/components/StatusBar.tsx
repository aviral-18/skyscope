...
export default function StatusBar() {
  ...
  return (
    <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[1000] max-w-[95vw] w-max">
      <div className="glass-panel rounded-2xl px-3 py-2 md:px-5 md:py-2.5 flex items-center gap-3 md:gap-5 text-[10px] md:text-xs shadow-lg">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white/30 hidden sm:inline">flights</span>
        </div>
        <div className="w-px h-4 bg-white/10" />
        <div className="flex items-center gap-2">
          <span className="text-white/30">↑</span>
          <span className="text-white/60 font-mono">{stats.airborne.toLocaleString()}</span>
          <span className="text-white/30 hidden sm:inline">airborne</span>
        </div>
        <div className="w-px h-4 bg-white/10" />
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-white/25 text-[9px] md:text-[10px] font-mono">· {freshLabel}</span>
        </div>
        <AnimatePresence>
